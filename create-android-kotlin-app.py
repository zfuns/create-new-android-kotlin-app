...
#REPO_URL = 'https://github.com/zfuns/create-new-android-kotlin-app'
REPO_URL = 'http://127.0.0.1:8080'
ORIGINAL_PACKAGE_NAME = 'create.kotlin.app'
ORIGINAL_PROJECT_NAME = 'create-android-kotlin-app'
ZIP_UNIX_SYSTEM = 3
...
class ProjectCreator:
    ...
    def create(self):
        ...
        for info in infolist:
            ...
            elif last_segment == 'strings.xml':
                self.string_xml(info)
    ...
    def string_xml(self, info: zipfile.ZipInfo):
        file_name = self.replace_root_path(info.filename)
        content = TextProcessor(self.zip_file.read(info.filename).decode('utf-8'))\
            .replace_all_text(ORIGINAL_PROJECT_NAME, self.project_name)\
            .remove_unwanted_comments().commit().encode()
        self.write_to_file(file_name, content, info)
...
def main():
    ...
    print('创建一个新的 android kotlin app 在 %s\n' % green("./" + project_name))
    print('从中获取最新的源代码存档 %s\n这可能需要几分钟时间.' % green(REPO_URL))
